...
from app.api.routes.v1.recipes.views.utility import get_recipes_categories_view, get_available_ingredients_view, \
    ...
    remove_recipe_from_likes_view, find_all_view
...
async def get_recipes(
    ...
    return await get_recipes_view(prefer_ingredients, exclude_groups, include_categories, session, current_user)
...
async def create_recipe(response: Response, title: str = Form(), image: UploadFile = File(default=None),
                        time: int = Form(),
...
async def get_available_ingredients(session: AsyncSession = Depends(manager.get_session_object)):
...
async def get_available_dimensions(session: AsyncSession = Depends(manager.get_session_object)):
...
async def get_available_ingredients_groups(session: AsyncSession = Depends(manager.get_session_object)):
    return await get_available_ingredients_groups_view(session)


@router.get("/utils/find", response_model=FindResponseModel)
async def find_all(string_to_find: str, max_returns: int = 5, session: AsyncSession = Depends(manager.get_session_object)):
