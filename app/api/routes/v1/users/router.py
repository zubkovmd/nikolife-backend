...
async def read_users_me(current_user: Users = Depends(get_current_active_user)):
    return UserRequestResponse(detail="Пользователь найден", user=User(**current_user.__dict__))


@router.get("/{user_id}", response_model=Union[UserRequestResponse, DefaultResponse])
async def get_user_by_id(
        user_id: int,
        session: AsyncSession = Depends(manager.get_session_object),
        current_user: Users = Depends(get_current_active_user)) -> Users:
